...
# --- صفحة البهارات بالترتيب الجديد المطلوب ---
elif st.session_state.page == 'spices':
    st.markdown('<div class="header-box"><h2>🌶️ نموذج البهارات</h2></div>', unsafe_allow_html=True)
    s_sels = []

    # 1. بهارات ناعمة 50 غ
    with st.expander("🌶️ بهارات ناعمة 50 غ", expanded=True):
        s_sels += render_standard(["بهار حلو", "فلفل أسود", "فلفل أحمر", "قرفة", "سبع بهارات", "دقة كعك", "كمون", "كزبرة", "كراوية", "كاري", "سماق", "يانسون", "عقدة صفراء", "فليفلة حلوة", "حامض", "ثوم مجفف", "شومر", "زعتر أوريغانو", "بفتاك", "بابريكا"], "s50", "50غ")

    # 2. بهارات 20 غ (ناعمة وحب) و بهارات حب 50 غ
    with st.expander("🌿 بهارات حب 50/20 غ و ناعمة 20 غ"):
        s_sels += render_standard(["-حب 50 غ", "بهار حلو", "فلفل أسود", "كمون", "كزبرة", "يانسون", "حبة البركة", "خميرة", "حبق", "لوما", "زنجبيل", "شومر", "حلبة"], "sh50")
        s_sels += render_standard(["-حب 20 غ", "جوزة الطيب", "محلب", "قرنفل", "هال", "عصفر"], "sh20")
        s_sels += render_standard(["-ناعمة 20 غ", "جوزة الطيب", "محلب", "نعنع", "قرنفل", "هال", "زنجبيل", "بهار أبيض", "مردكوش"], "sn20")

    # 3. أصناف متنوعة / تعبئة مختلفة
    with st.expander("📋 أصناف متنوعة / تعبئة مختلفة"):
        s_sels += render_standard(["-بيكربونات", "بيكربونات (100 غ × 12)", "بيكربونات (500 غ)", "بيكربونات (1000 غ)", "-حامض", "حامض (100 غ × 12)", "حامض (200 غ × 12)", "حامض (500 غ)", "حامض (1000 غ)", "-سماق", "سماق (200 غ)", "سماق (500 غ)", "سماق (1000 غ)", "-يانسون حب", "يانسون حب (200 غ)", "يانسون حب (500 غ)", "يانسون حب (1000 غ)"], "smisc")

    # 4. بهارات ناعمة 500 غ
    with st.expander("🌶️ بهارات ناعمة 500 غ"):
        s_sels += render_standard(["بهار حلو", "فلفل أسود", "فلفل أحمر", "قرفة", "سبع بهارات", "دقة كعك", "كمون", "كزبرة", "كراوية", "كاري", "يانسون", "عقدة صفراء", "فليفلة حلوة", "بابريكا", "ثوم مجفف", "شومر", "جوزة الطيب", "محلب", "نعنع", "مردكوش", "قرنفل", "هال", "زنجبيل", "بهار أبيض", "عصفر", "صبغة حمراء", "شبة", "خولجان", "حلبة"], "s500n", "500غ ناعم")

    # 5. بهارات حب 500 غ
    with st.expander("🌿 بهارات حب 500 غ"):
        s_sels += render_standard(["بهار حلو", "فلفل أسود", "فلفل أسود خشن", "فلفل أحمر خشن", "قرفة", "دقة كبة", "دقة كبة مجروشة", "دقة نفسا", "كمون", "كراوية", "يانسون", "جوزة الطيب", "محلب", "قرنفل", "هال", "زنجبيل", "خلطة للسلق", "شومر", "حبة البركة", "ورق غار", "لوما", "ورد", "بابونج", "حبق", "خميرة", "كزبرة"], "s500h", "500غ حب")
