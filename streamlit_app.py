...
# --- صفحة الحبوب (تم فصل العناوين) ---
    ...
    with st.expander("📦 تعبئة 1000غ / 907غ", expanded=True):
        g_sels += render_standard([
            ...
            "-طحين", "طحين فرخة (907غ)", "سميد (907غ)", "غود ميدل (1000غ)", "طحين زيرو (1000غ)", "طحين فقش (1000غ)", 
    ...
    with st.expander("📦 تعبئة 500غ / 454غ"):
        g_sels += render_standard([
            "-سمسم", "سمسم مقشور", "سمسم محمص", "سمسم بلدي محمص", 
            "-نشاء", "نشاء ناعم", "نشاء حب", 
            "-زعتر", "زعتر محوج", "زعتر اكسترا", "زعتر حلبي", 
            "-مغلي", "مغلي جاهز", "مغلي بدون سكر", "مهلبيه", "مهلبيه ظرف", "سحلب", 
            "-خلطات", "خلطة كرسبي", "خلطة بروستد", 
            "-ذرة", "ذره بوشار (454غ)", "ذره مجروشه (500غ)", 
            "-ترمس", "ترمس حلو (500غ)", "ترمس مر (500غ)", 
            "-سكر", "سكر ناعم", "سكر نبات", "سكر اسمر", 
            "-مختلف", "شوفان مبروش", "شوفان حب", "فاصوليا عريضة", "فريك مجروش", "فول عريض", "برش الهند", "أرز ناعم", "ملوخيه", "كعك مطحون"
        ], "g500_final", "500غ")
    ...
    with st.expander("📋 تعبئة مختلفة"):
        g_sels += render_standard([
            ...
            "-علب", "فانيليا 20 غ × 12", "باكينغ بودر 20 غ × 12", 
            "-كرتون", "صنوبر × 12", "مسكة حب × 25", 
            "-زهورات", "زهورات 100 غ × 12", "زهورات 200 غ × 12", 
            "-قمح", "مقشور 2 كلغ", "مقشور 5 كلغ", 
            "-مختلف", "بابونج 100 غ × 12", "بطاطا شيبس 100 غ", "بامية زهرة 100 غ", "كاكاو 100 غ"
        ], "gmisc_final")
...
# --- صفحة البهارات (سيتم تجهيزها فوراً) ---
elif st.session_state.page == 'spices':
    st.markdown('<div class="header-box"><h2>🌶️ نموذج البهارات</h2></div>', unsafe_allow_html=True)
    st.info("نحن الآن بانتظار تأكيدك لصفحة الحبوب لنقوم ببناء البهارات بنفس هذا التنسيق المفصل.")
    if st.button("🔙 عودة"): st.session_state.page = 'home'; st.rerun()
